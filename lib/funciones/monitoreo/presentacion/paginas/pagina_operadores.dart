...
class PaginaOperadores extends StatefulWidget {
  const PaginaOperadores({super.key, this.empresaFiltro});
        ...
        builder: (context, snapshotPos) {
          if (snapshotPos.hasData) {
            for (var p in snapshotPos.data!) {
              ultimosReportes[p['fk_emisor'].toString()] = DateTime.parse(
                p['tiempo'],
              );
            }
          }
          ...
          return StreamBuilder<List<ModeloEquipo>>(
            ...
            builder: (context, snapshotEquipos) {
              ...
              if (!snapshotEquipos.hasData || snapshotEquipos.data!.isEmpty) {
                return const Center(
                  child: Text(
                    "No se encontraron operadores Tracker",
                  ...
                  if (equipo.activo) {
                    ...
                    if (ultimosReportes.containsKey(equipo.id)) {
                      final diff = DateTime.now()
                          .difference(ultimosReportes[equipo.id]!)
                          .inSeconds;
                      ...
                      children: [
                        ...
                        GestureDetector(
                          onTap: () => Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => PaginaPerfilEquipo(
                                equipo: equipo,
                                empresaFiltro: _empresaFiltroActual,
                              ),
                            ),
                          ),
                          child: CircleAvatar(
      ...
      children: [
        if (!equipo.activo)
          _buildBotonCompacto(
            ...
            color: Colors.green,
            onPressed: () =>
                _repositorio.actualizarEquipo(equipo.id, {'fecha_final': null}),
          ),
        if (equipo.activo)
          _buildBotonCompacto(
            label: "FINALIZAR",
            color: Colors.red,
            onPressed: () => _repositorio.actualizarEquipo(equipo.id, {
              'fecha_final': DateTime.now().toIso8601String().split('T').first,
            }),
          ),
      ],
    );
